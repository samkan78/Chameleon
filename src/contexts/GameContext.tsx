...
export const GameProvider: React.FC<{children: ReactNode}> = ({ children }) => {
    ...
    const handleActionClick = (action: Action) => {
        ...
        if (napActive && !isShopOrEarn) {
            open(<div>Your chameleon is napping — wait until it wakes up.</div>, 3000);
            return;
        }
        ...
        if (action.healthValue) {
            if (health + action.healthValue > 100) {
                setHealth(100);
                setHappiness(h => h - 5);
                open(<div>Health is at maximum! Your chameleon gets restless.</div>, 3000);
                return;
            } else {
        ...
        if (action.isFood) {
            ...
            open(<div>You have purchased food! You now have {foodInventory + 1} food items.</div>, 3000);
            return;
        }
        ...
        if (action.name === "Nap" && action.cooldown) {
            ...
            open(<div>Your chameleon is now napping for {action.cooldown} minutes.</div>, 3000);
        }
