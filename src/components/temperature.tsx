...
const TemperatureBar: React.FC<TemperatureBarProps> = ({
  ...
  const timeUntilChange = tempChangeUntil
    ? Math.max(0, tempChangeUntil - Date.now())
    : 0;

  let color = "#60a5fa";
  if (isInGoodZone) color = "#1fcc1f"; // Green for good zone
  else if (temperature >= 85) color = "#a02914"; // Red for too hot
  else if (temperature >= 70) color = "#cc7533"; // Orange for warm
