...
type Action = {
  ...
  isMedicine?: boolean; // whether the action is medicine to be used in inventory
  hasMedicine?: boolean; // whether the action involves medicine
  isNap?: boolean; // whether the action is a nap, if so energy  increases and all actions disabled for cooldown period
};
...
const actions: Record<Category, Action[]> = {
  Health: [
    ...
    { name: "Vet Visit", cost: 75, healthValue: 50, happinessValue: -25, hydrationValue: 30, energyValue: -10, hungerValue: 30  }, //vet visit is expensive but greatly improves health, hydration, hunger while using some energy and decreasing happiness due to stress
    { name: "Administer Medication", healthValue: 25, happinessValue: -10, hasMedicine:true }, //administering medication increases health but decreases happiness, hasMedicine indicates medicine is involved and is used to make sure it is only clickable when medicine is available in inventory and consumes medicine item
  ],
  Care: [
    { name: "Bath", healthValue: 10, happinessValue: -10, cooldown: 5 }, //bathing increases health but decreases happiness due to disturbance
    ...
    { name: "Nap", energyValue: 30}, //napping increases energy, cooldown in 2 minutes
  ...
  Shop: [
    ...
    { name: 'Medical Kit', cost: 30,  }, //buying medical kit for health purposes
  ],
  Earn: [
    { name: "Clean Room", cost: -20, cooldown:5}, //earning coins by doing chores, negative cost indicates earning coins, cooldown in minutes
    { name: "Do Homework", cost: -30,cooldown:10 }, //earning coins by doing chores, negative cost indicates earning coins, cooldown in minutes
    { name: "Take a shower", cost: -45, cooldown:15 }, //earning coins by doing chores, negative cost indicates earning coins, cooldown in minutes
    { name: "Do laundry", cost: -45, cooldown:20 },  //earning coins by doing chores, negative cost indicates earning coins, cooldown in minutes
  ],
};
...
const FourButtons: React.FC<FourButtonsProps> = ({
  ...
  const [medicines, setMedicines] = useState(0) //inventory state for medicine items
  ...
  const handleActionClick = (action: Action) => {
    ...
    if (action.isMedicine) {
      setMedicines(medicines + 1); //increasing medicine count by 1
      alert(`You have purchased medicine! You now have ${medicines + 1} medicine items in your inventory.`); //alert to inform user of medicine count
      return;
    }
    //Food count
    if (action.isFood) {
