...
const actions: Record<Category, Action[]> = {
  Health: [
    ...
    { name: "Clean Enclosure", healthValue: 10, happinessValue: -10 }, //cleaning enclosure improves health but decreases happiness due to disturbance
    {
      ...
      energyValue: -10,
      hungerValue: 30,
    }, //vet visit costs coins but greatly improves health, hydration, hunger and slightly decreases energy and happiness due to stress
  ],
  Care: [
    { name: "Bath" },
...
const FourButtons: React.FC = () => {
  ...
  const handleActionClick = (action: Action) => {
    ...
    if (action.healthValue !== undefined) {
      if (health + action.healthValue > 100) {
        ...
        setHappiness(happiness - 5), //chameleon gets restless with too much disturbance
          open(
            <div className="bg-red-500 text-white px-4 py-3 rounded-lg shadow-lg">
              Health is at maximum! Your chameleon gets restless with too much
              disturbance.
            </div>,
            3000
    ...
    if (
      ...
      open(
        <div className="bg-green-500 text-white px-4 py-3 rounded-lg shadow-lg">
          Tier 2 Tricks Unlocked! You can now teach your chameleon Fetch and
          Target Training.
        </div>,
        3000
      );
    ...
    if (action.name === "Nap" && action.cooldown) {
      //checking if action is nap and has cooldown defined
    ...
    if (action.cooldown !== undefined && action.cooldown > 0) {
      setCooldowns((prev) => ({
        ...prev,
        [action.name]: Date.now() + (action.cooldown as number) * 60 * 1000,
      }));
    }
        ...
        <div className="sub-buttons">
          ...
          {actions[active].map((action) => {
            const cannotAfford =
              ...
              action.cost > 0 &&
              coins < action.cost; // disable if not enough coins
            const isTier2LockedTrick =
              action.tiertwotrick === true && !trickt2unlocked; // disable if tier 2 trick not unlocked
            const isAlreadyUsed =
              action.hasLock && lockedActions.has(action.name); // disable if locked action already used
            ...
            const disabled =
              ...
              isTier2LockedTrick ||
              isAlreadyUsed ||
              (napActive && !isShopOrEarn) ||
              hasCooldownActive; // overall disabled state
            ...
            return (
              ...
              >
                ...
                {/* earning coins if negative cost */}
                {actions.Earn.some((a) => a.name === action.name) &&
                  hasCooldownActive &&
                  ` — ${formatMs(cooldownRemaining)}`}{" "}
                {/* show cooldown timer for earn actions */}
              </button>
            );
          })}
        </div>
